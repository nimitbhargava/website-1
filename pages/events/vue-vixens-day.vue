<template>
  <div>
    ...
    <!-- info section -->
    <v-container pa-5>
      <h1 class="text-xs-center ma-3 primary--text font-lato">{{event.title}}</h1>
      <v-layout row wrap justify-center>
        <v-flex xs12 sm6 px-5 d-flex align-center v-html="toHtml(event.description)"></v-flex>
        <v-flex xs12 sm6 px-5 v-if="event.schedule">
          <h3 class="headline-1 mb-2">Schedule</h3>
          <ul class="pa-0">
            <li v-for="event in event.schedule" v-bind:key="event.activity">
              <span class="font-weight-medium">{{event.time}}</span>
              {{event.activity}}
            </li>
          </ul>
        </v-flex>
    ...
    <div v-if="event.speakers" class="secondary darken-2">
      <v-container pa-5>
        ...
        <v-layout wrap justify-space-around>
          <v-flex xs12 sm3 md2 pa-2 v-for="speaker in event.speakers" v-bind:key="speaker.name">
            <v-card>
              <v-img
                height="150px"
    ...
    <v-container v-if="event.mentors" pa-5>
      ...
      <v-layout wrap justify-space-around>
        <v-flex xs12 sm3 md2 pa-2 v-for="mentor in event.mentors" v-bind:key="mentor.name">
          <v-card>
            <v-img
              height="150px"
    ...
    <v-container v-if="event.sponsors" pa-5>
      ...
      <div v-if="event.sponsors.categories">
        <v-container v-for="category in event.sponsors.list" v-bind:key="category.name">
          <h2 class="headline text-xs-center ma-2">{{category.name}}</h2>
          <v-layout row wrap justify-space-around>
            <v-flex xs12 sm4 md3 v-for="sponsor in category.value" v-bind:key="sponsor.name">
      ...
      <div v-else>
        <v-layout row wrap justify-space-around>
          <v-flex xs12 sm3 v-for="sponsor in event.sponsors.value" v-bind:key="sponsor.name">
            <div class="vv-day-sponsor px-4 mb-3">
          ...
          list: [
            {
              ...
              value: [
                ...
                },
                {
                  name: "Monoku",
                  logo:
                    "https://cdn-images-1.medium.com/max/1600/1*8efy7GSRHiI_UoypVgn9LA.jpeg",
              ...
              value: [
                {
                  name: "Huge inc",
                  logo:
                    "https://pbs.twimg.com/profile_images/428773574132772864/9nxNTbHA_400x400.jpeg",
